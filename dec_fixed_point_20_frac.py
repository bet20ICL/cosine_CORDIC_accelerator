def decimal_to_fixed_point_binary(decimal_num):
    fixed_point_num = int(decimal_num * 2**20) # fixed_point_num = int(decimal_num * 2**31)

    if fixed_point_num > 2**20 - 1 or fixed_point_num < -2**20:
        raise ValueError("Fixed-point number is out of range.")

    binary_str = bin(fixed_point_num & (2**21 - 1))[2:].zfill(21)
...
for i, element in enumerate(cof16):
    ...
    print("4'd{}   : rotateAngle = 21'b{};".format(i,fixed_point_binary)) 
